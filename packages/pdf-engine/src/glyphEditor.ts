...
export function glyphPreservingEdit(
  ...
  try {
    ...
    const processStream = (streamObj: PDFObject): boolean => {
      // streamObj is always a PDFStream subclass at runtime but the pdf-lib public
      // interface doesn't expose getContentsString / dict / setUnencodedBytes
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const stream = streamObj as unknown as { getContentsString(): string };
      ...
      try {
        ...
        const pdfStream = streamObj as unknown as { dict: { context: { obj(v: unknown): PDFObject }; set(n: PDFName, v: unknown): void } };
        const streamDict = pdfStream.dict;
        if (streamDict) {
        ...
        try {
          ...
          // setUnencodedBytes is only on PDFFlateStream — use unknown cast
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
