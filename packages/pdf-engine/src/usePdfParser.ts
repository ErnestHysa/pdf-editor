...
export class PdfParser {
  ...
  async parsePage(pageIndex: number): Promise<ParsedTextObject[]> {
    ...
    for (const item of textContent.items) {
      // pdf.js TextItem has many optional properties (str, transform, width, height,
      // fontName, color, etc.) not all present in the public type definitions
      const op = item as unknown as {
        str?: string;
  ...
  async parseAllPages(onProgress?: (current: number, total: number) => void): Promise<Map<number, ParsedTextObject[]>> {
    ...
    for (let offset = 0; offset < pageCount; offset += CHUNK_SIZE) {
      ...
      const parsedCount = Math.min(offset + CHUNK_SIZE, pageCount);
      onProgress?.(parsedCount, pageCount);
  ...
  async detectFormFields(): Promise<FormField[]> {
    ...
    try {
      ...
      for (let i = 0; i < pdfDoc.numPages; i++) {
        ...
        // getAnnotations() is not in the official PDFPageProxy type but is available at runtime
        const annotations = await (page as unknown as { getAnnotations(): Promise<unknown[]> }).getAnnotations();
        for (const ann of annotations) {
          // PDF widget annotation properties — not fully typed in pdfjs-dist public API
  ...
  async detectMissingFonts(): Promise<string[]> {
    ...
    try {
      ...
      // fontList is a pdf.js internal property containing loaded font info.
      // Not in the public PDFDocumentProxy type but present at runtime.
      const fontList = (pdfDoc as unknown as { fontList?: Array<{ name?: string; embedded?: boolean }> }).fontList ?? [];
      for (const fontRef of fontList) {
        // Font ref has runtime shape { name?: string; embedded?: boolean }
        const fontInfo = fontRef as unknown as { name?: string; embedded?: boolean };
